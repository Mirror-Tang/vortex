...
## 机密GPU的CC(Confidential Computing)⼯作模式

CC⼯作模式                                              含义
Off                                                 常规的GPU⼯作模式
On                                               开启机密计算的保护模式
Devtools                关闭机密计算保护模式，但驱动还按照CC⼯作模式开启的⽅式运⾏；GPU性能计数器将不再被禁⽤

• CC⼯作模式必须在GPU启动前提前配置好。在云场景中由VMM来设置；将CC⼯作模式位配置到GPU EEPROM中，然后执⾏GPU reset才能⽣效。
...
• FIPS 140-3 Level 2密码加密：用于防止未授权访问加密密钥
